...
export function UpdateDialog() {
  ...
  const progress = percent(download);
  ...
  return (
    <Dialog open={open && !!release} onOpenChange={showUpdate}>
      <DialogContent className="top-[12%] flex max-h-[76vh] max-w-lg flex-col">
        ...
        <DialogDescription>
          {date && `Released ${date}. `}
          {mode === "download" ? "This copy of GitViber can't replace itself: get the new version from the Releases page." : "Nothing changes until you restart."}
        </DialogDescription>
        ...
        <div className="mt-4 flex justify-end gap-2">
          ...
          {mode === "download" ? (
            ...
            <Button disabled={restarting} onClick={() => void restartToUpdate()}>
...
export function UpdateButton() {
  ...
  const progress = percent(download);
  const label = download === "ready" ? (restarting ? "Restarting…" : "Restart to Update") : download ? `Downloading update…${progress === null ? "" : ` ${progress}%`}` : `Update to v${release.version}`;
  return (
