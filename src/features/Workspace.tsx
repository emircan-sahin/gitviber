            ...
            <div className="flex h-full flex-col bg-panel">
              ...
              <div className="min-h-0 flex-1">
                {listTab === "changes" && status && (
                  <ChangesPanel status={status} head={repo.commits[0] ?? null} activeKey={activeKey} onOpen={open} onHover={prefetch} refresh={() => repo.refresh(false)} viewed={viewed} setViewed={setViewed} onRevealInExplorer={revealInExplorer} />
                )}
                {listTab === "pulls" && (
                  <PullsPanel
                    status={status}
