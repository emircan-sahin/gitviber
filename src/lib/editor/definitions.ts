...
// (lib/links/links) goes to the file it names. URLs are Monaco's own links.
...
import { type LinkSide, type LinkTree, openTarget, resolverFor } from "../links/linkHost";
import { findLinks } from "../links/links";
...
export function followDefinitions(editor: monaco.editor.ICodeEditor, side: () => LinkSide | null): monaco.IDisposable {
  // A model outlives the view (lib/editor/monaco keeps it): its entry goes when it leaves this editor,
...
monaco.editor.registerLinkOpener({
  open(uri) {
    ...
    github.openUrl(uri.toString(true)).catch(failed("Could not open the link"));
    return true;
  },
});
