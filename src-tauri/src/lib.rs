...
/// open_repo's error for an existing folder outside any repository; the page offers `git init`.
const NOT_A_REPO: &str = "git:not-a-repo";

#[tauri::command]
async fn open_repo(app: AppHandle, state: State<'_, AppState>, path: String) -> Res<OpenedRepo> {
    let (root, main) = blocking(move || {
        let path = PathBuf::from(path);
        let root = git::toplevel(&path).map_err(|e| {
            if path.is_dir() {
